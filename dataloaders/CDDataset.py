...
class ImageDataset(BaseDataSet):
    ...
    def _load_data(self, index):
        image_A_path = os.path.join(self.root, 'A', self.files[index]).strip()
        image_B_path = os.path.join(self.root, 'B', self.files[index]).strip()
        label_path = os.path.join(self.root, 'label', self.files[index]).strip()
